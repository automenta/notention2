...
async function loadNotes() {
    ...
    if (!notes.size) {
        devNotes.concat(seedNote).forEach(n => {
            console.log(n);
            notes.set(n.id, Note.parse(n));
        });
        for (const [id, note] of notes) await writeFile(join(NOTES_DIR, `${id}.json`), JSON.stringify(note));
    }
}
...
async function runNote(noteId) {
    ...
    for (const step of note.logic) {
        ...
        if (tool) {
            ...
            // Chain: Update next step input if applicable
    ...
    wss.on('connection', async (ws) => {
        ...
        ws.on('message', async (msg) => {
            ...
            if (type === 'createNote') {
                ...
                const note = Note.parse({id, title: data.title, content: ''});
                notes.set(id, note);
            ...
            if (type === 'updateNote') {
                ...
                const updated = Note.parse({...note, ...data});
